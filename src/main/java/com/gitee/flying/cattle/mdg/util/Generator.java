...
public class Generator {
	...
	//修改6---定义生成模版的方法
	//生成REPOSITORY
	public static ResultJson createRepository(String url, BasisInfo bi) {
		String fileUrl= getGeneratorFileUrl(url,bi.getRepositoryUrl(), bi.getEntityName(), REPOSITORY);
		return FreemarkerUtil.createFile(bi, "repository.ftl", fileUrl);
	}
	//生成文件路径和名字
	public static String getGeneratorFileUrl(String url,String packageUrl,String entityName, String type){
		if (type.equals("entity")) {
			return url+pageToUrl(packageUrl)+entityName+"Entity.java";
			...
			return url+pageToUrl(packageUrl)+entityName+"Repository.java";
		}
		return null;
	}
